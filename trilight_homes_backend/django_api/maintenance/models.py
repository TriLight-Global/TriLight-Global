...
from property_listing.models import AuditFields, Property
 
User = get_user_model()

class PropertyMaintenance(AuditFields):
    """
    Model representing maintenance records for a property.

    Attributes:
        property (ForeignKey): The property requiring maintenance.
        description (TextField): Description of the maintenance issue.
        reported_date (DateTimeField): Date and time the issue was reported.
        completed_date (DateTimeField): Date and time the maintenance was completed.
        status (CharField): Current status of the maintenance request.
    """
    STATUS_CHOICES = [
        ('REPORTED', 'Reported'),
    ...
    def __str__(self):
        return f"Maintenance for {self.property.title} - Status: {self.get_status_display()}"
